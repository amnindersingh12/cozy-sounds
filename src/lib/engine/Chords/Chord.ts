...
class Chord {
    ...
    nextChordIdx(rng = Math.random) {
        return this.nextChordIdxs[Math.floor(rng()*this.nextChordIdxs.length)];
    }

    generateVoicing(size, rng = Math.random) {
        if(size<3)
            return this.intervals.slice(0,3);
        let voicing = this.intervals.slice(1,size);
        voicing.sort(() => rng()-0.5);
