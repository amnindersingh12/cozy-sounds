...
def train(dataset, model, name, max_epochs=None):
    ...
    while max_epochs is None or epoch < max_epochs:
        ...
        for batch, data in enumerate(train_dataloader):
            ...
            loss_chords_value = float(loss_chords.detach().cpu().item())
            loss_melody_value = float(loss_melody.detach().cpu().item()) if hasattr(loss_melody, "detach") else float(loss_melody)
            kl_loss_value = float(kl_loss.detach().cpu().item())

            ep_train_losses_chords.append(loss_chords_value)
            ep_train_losses_melodies.append(loss_melody_value)
            ep_train_losses_kl.append(kl_loss_value)
        ...
        for batch, data in enumerate(val_dataloader):
            with torch.no_grad():
                ...
                ep_val_losses_chords.append(loss_chords_value)
                ep_val_losses_melodies.append(loss_melody_value)
                ep_val_losses_kl.append(kl_loss_value)
        ...
        if os.environ.get("PLOT_SHOW", "0") == "1":
            plot.show()
        else:
            plot.close(fig)
